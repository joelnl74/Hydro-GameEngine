...
#include <fbxsdk/fbxsdk_nsbegin.h>
		...
		// We are building using VisualStudio on 32 bits. We ignore the warnings generated
		// by the /Wp64 option. This code is warning free when truly compiling in 64bits
		#pragma warning(push)
		#pragma warning(disable: 4311)
		#pragma warning(disable: 4312)
		#pragma warning(disable: 4996)
		#if _MSC_VER >= 1900 && defined(_WIN64)
            // this warning occurs when building 64bit. I guess the void* are too big to
            // be casted in long (possible trouble!!!)
			#pragma warning( disable : 4302)
            #pragma message ("Disabled 4302. Code contains cast from void* to long. May become problematic --> " __FILE__)
		#endif
	#endif
#endif
        ...
        case XML_RELAXNG_CHOICE:{
		     ...
		     */
                    ...
                    if ((node->type == XML_TEXT_NODE) ||
                        (node->type == XML_CDATA_SECTION_NODE)) {
                        list =
                            (xmlRelaxNGDefinePtr) xmlHashLookup2(triage, BAD_CAST "#text", NULL);
                    } else if (node->type == XML_ELEMENT_NODE) {
                        if (node->ns != NULL) {
                            list = (xmlRelaxNGDefinePtr) xmlHashLookup2(triage, node->name,
                                                  node->ns->href);
                            if (list == NULL)
                                list =
                                    (xmlRelaxNGDefinePtr) xmlHashLookup2(triage, BAD_CAST "#any",
                                                   node->ns->href);
                        } else
                            list =
                                (xmlRelaxNGDefinePtr) xmlHashLookup2(triage, node->name, NULL);
                        if (list == NULL)
                            list =
                                (xmlRelaxNGDefinePtr) xmlHashLookup2(triage, BAD_CAST "#any",
                                               NULL);
