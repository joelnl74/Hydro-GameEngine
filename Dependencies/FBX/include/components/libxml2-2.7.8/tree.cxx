...
xmlUnlinkNode(xmlNodePtr cur) {
	...
	if (doc != NULL) {
	    if (doc->intSubset != NULL) {
		    ...
		    xmlHashRemoveEntry((xmlHashTablePtr)doc->intSubset->entities, cur->name,
		                       NULL);
	        if (xmlHashLookup((xmlHashTablePtr)doc->intSubset->pentities, cur->name) == cur)
		    xmlHashRemoveEntry((xmlHashTablePtr)doc->intSubset->pentities, cur->name,
		                       NULL);
	    }
	    if (doc->extSubset != NULL) {
	        if (xmlHashLookup((xmlHashTablePtr)doc->extSubset->entities, cur->name) == cur)
		    xmlHashRemoveEntry((xmlHashTablePtr)doc->extSubset->entities, cur->name,
		                       NULL);
	        if (xmlHashLookup((xmlHashTablePtr)doc->extSubset->pentities, cur->name) == cur)
		    xmlHashRemoveEntry((xmlHashTablePtr)doc->extSubset->pentities, cur->name,
		                       NULL);
	    }
	}
    }
