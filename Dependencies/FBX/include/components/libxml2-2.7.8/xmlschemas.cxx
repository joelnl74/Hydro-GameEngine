                     ...
                     const xmlChar * context ATTRIBUTE_UNUSED)
{
    ...
    if (_namespace != NULL)
	fprintf(output, "ns '%s'", _namespace);
    fprintf(output, "\n");
#if 0
...
#define WXS_FIND_GLOBAL_ITEM(slot)			\
	...
	ret = xmlHashLookup((xmlHashTablePtr)schema->slot, name); \
	if (ret != NULL) goto exit; \
    } \
    if (xmlHashSize((xmlHashTablePtr) schema->schemasImports) > 1) { \
	xmlSchemaImportPtr import; \
	if (nsName == NULL) \
	    import = (xmlSchemaImportPtr) xmlHashLookup((xmlHashTablePtr) schema->schemasImports, \
		XML_SCHEMAS_NO_NAMESPACE); \
	else \
	    import = (xmlSchemaImportPtr) xmlHashLookup((xmlHashTablePtr) schema->schemasImports, nsName); \
	if (import == NULL) \
	    goto exit; \
	ret = xmlHashLookup((xmlHashTablePtr) import->schema->slot, name); \
    }	
...
{
	void * ret = NULL;
    if ((name == NULL) || (schema == NULL)) return xmlSchemaElementPtr(NULL);
...
#endif
    return xmlSchemaElementPtr(ret);
}
                 ...
                 const xmlChar * nsName)
{
    ...
    if (name == NULL)
        return xmlSchemaTypePtr(NULL);
    /* First try the built-in types. */
    if ((nsName != NULL) && xmlStrEqual(nsName, xmlSchemaNs)) {
	ret = (void *)xmlSchemaGetPredefinedType(name, nsName);
	if (ret != NULL)
	    goto exit;
	/*
                 ...
                 const xmlChar * nsName)
{
    ...
    if ((name == NULL) || (schema == NULL))
        return xmlSchemaAttributePtr(NULL);
    if (schema != NULL) {
                 ...
                 const xmlChar * nsName)
{
    ...
    if ((name == NULL) || (schema == NULL))
        return xmlSchemaAttributeGroupPtr(NULL);
    if (schema != NULL) {
                 ...
                 const xmlChar * nsName)
{
    ...
    if ((name == NULL) || (schema == NULL))
        return xmlSchemaModelGroupDefPtr(NULL);
    if (schema != NULL) {
		     ...
		     const xmlChar *nsName)
{
    ...
    if ((name == NULL) || (schema == NULL))
        return xmlSchemaNotationPtr(NULL);
    if (schema != NULL) {
